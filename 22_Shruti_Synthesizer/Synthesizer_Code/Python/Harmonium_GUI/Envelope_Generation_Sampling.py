...
class Envelope_Generation_Sampling():
    ...
    def Generate_Linespace(self,x1,x2,t):
    ...
    def Generate_Sound(self,f,s,n,d):
        
        data, fs, enc = wavread(self.wav)
        ...
        bf = 263.1   # Base frequency of the Note Sample in Hz
        ...
        return self.note_making(bf, f1, data, d, fs)

    def Generate_Sound_Western(self,f,s,n,d):
        ...
        f1 = f*np.power(2,float(n)/float(12))

        return self.note_making(bf, f1, data, d, fs)

    def note_making(self,bf, f1, data, d, fs):
        ...
        T = np.ceil(fs * d); # Time Period

        final_tone = resample(data[:,0], resample_fraction, "sinc_fastest")

        final_tone = final_tone[0:int(T)]

        T1 = np.ceil(0.0 * T); # for linspace - Attack
        T2 = np.ceil(0.0 * T); # for linspace - Decay
        T3 = np.ceil(1 * T); # for linspace - Sustain
        T4 = np.ceil(0.0 * T); # for linspace - Release

        L1 = self.Generate_Linespace(0,0.80,T1)
        L2 = self.Generate_Linespace(0.80,0.80,T2)
        L3 = self.Generate_Linespace(0.80,0.80,T3)
        L4 = self.Generate_Linespace(0.80,0,T4)
        ...
        final_tone = self.max_sample*final_tone # Normalizing for converting int16 from float 32 in future
